...
- `GET /device` (`show`), `PUT /device/:id/update`, and `DELETE /device/:id` are placeholders that return `501`. `GET /device/:id/edit` still serves static sample data.
...
## Data Layer
- Sequelize/MySQL is configured in `src/database/database.ts`, and `src/models/device.ts` is used by the home index and create flows.
...
## Views
- `index.ejs` renders database-backed device cards. Devices with an `external_url` receive an external action; the Wake-on-LAN action remains disabled and is not implemented.
